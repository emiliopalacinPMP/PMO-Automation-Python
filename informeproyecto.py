...
try:
    ...
    print("❌ Error: Sube el archivo 'Detalle_CurvaS_Subestacion.xlsx'.")
    raise
...
# Crear etiquetas cortas para gráficos (ej: F1, F2...)
...
# Definición de la Rejilla (GridSpec)
...
txt_report = (
    f"INFORME EJECUTIVO FINAL - PROYECTO SUBESTACIÓN 10MVA\n"
    f"=====================================================\n\n"
    ...
    f"2. ANÁLISIS DE CAUSA RAÍZ (FASES CRÍTICAS):\n"
    f"------------------------------------------\n"
    f"  La fase '{fase_critica['Label']}' presenta la mayor\n"
    f"  desviación negativa (${fase_critica['CV']:,.0f}).\n\n"
    f"  Hallazgos:\n"
    f"  - El CPI de {fase_critica['CPI']:.2f} indica que por cada\n"
    f"    dólar invertido, solo se genera ${fase_critica['CPI']:.2f}.\n"
    f"  - Tendencia: El gráfico histórico muestra una\n"
    f"    caída sostenida del rendimiento desde el Mes 3.\n\n"
    f"3. RECOMENDACIONES ESTRATÉGICAS:\n"
    f"--------------------------------\n"
    f"  A. ACCIÓN INMEDIATA (CORTO PLAZO):\n"
    f"    - Realizar auditoría de rendimientos en {fase_critica['Label']}.\n"
    f"    - Congelar cambios de alcance.\n\n"
    f"  B. GESTIÓN CONTRACTUAL:\n"
    f"    - Revisar cláusulas de penalización.\n"
    f"    - Evaluar recuperación de cronograma (Crashing).\n\n"
    f"  C. PROYECCIÓN:\n"
...
# --- D. GRÁFICO DE LÍNEAS (TOP RIGHT QUADRANT - EVOLUCIÓN) ---
ax_line = fig.add_subplot(gs[1, 2])

# Zona crítica inferior
ax_line.axhspan(0.7, 0.9, color=c_zone_red, alpha=0.5, label='Zona Crítica')
...
ax_line.axhline(1.0, color='#4CAF50', linestyle='--', label='Línea Base (1.0)', linewidth=1.5)

ax_line.set_title("EVOLUCIÓN HISTÓRICA DE ÍNDICES DEL PROYECTO", fontsize=9, pad=8)
...
# --- E. GRÁFICO DE BURBUJAS (BOTTOM LEFT QUADRANT - MAPA) ---
...
ax_bub.set_title("MAPA DE DESEMPEÑO (BURBUJAS)", fontsize=9, weight='normal')
...
# --- F. GRÁFICO DE BARRAS (BOTTOM RIGHT QUADRANT - ANÁLISIS) ---
...
ax_bar.set_title("ANÁLISIS DE VARIANZA FINANCIERA (CV VS SV)", fontsize=9, pad=8)
